...
function buttonk(k) {
    ...
    var inputs = document.querySelectorAll('.button-input-selected')
    ...
    $('.button-input-selected')
        ...
        .removeClass('button-input')
    r = k.value
    changeR(r)

    var input = document.querySelector('.button-input-selected')
    var g = input.getAttribute('id')
    switch (g) {
        case 'input-form:r1':
            $(".hidden-r1").click()
            break;
...
function drawPlot() {
    ...
    canvas.width = 300;
    canvas.height = 300;
    ctx.fillStyle = "rgba(136, 79, 201, 0.5)";
    ctx.strokeStyle = "rgb(136, 79, 201)";
    ctx.beginPath();
    ctx.moveTo(150, 150);
    ctx.lineTo(150 + dr, 150);
    ctx.lineTo(150 + dr, 150 + dr / 2);
    ctx.lineTo(150, 150 + dr / 2);
    ctx.lineTo(150, 150 + dr);
    ctx.lineTo(150 - dr, 150);
    ctx.arc(150, 150, dr, -Math.PI, -Math.PI / 2);
    ctx.lineTo(150, 150);
    ctx.fill();
    ctx.stroke();
...
function drawPoints() {
    $("#result-table tr").each(function () {
        ...
        if (isNaN(x) || isNaN(y) || isNaN(_r)) return;
        drawPoint(x, y, _r, res)
    })
}
